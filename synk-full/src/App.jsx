...
IMPORTANT RULES:
1. Be conversational and natural - don't repeat the same phrases
2. When adding assignments, use the JSON format below, then confirm briefly
3. For questions, just answer naturally without JSON
4. Remember what the user said earlier in the conversation
5. Parse dates relative to today (${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })})

To ADD assignments, include this JSON block:
\`\`\`json
{"action":"add","assignments":[{"title":"Title here","class":"Class name","category":"math","date":"YYYY-MM-DD","time":"11:59 PM"}]}
\`\`\`

To DELETE, use the assignment ID from the list:
\`\`\`json
...
Date parsing examples for today (${new Date().toLocaleDateString()}):
- "Friday" → calculate the next Friday's actual date
- "tomorrow" → ${new Date(Date.now() + 86400000).toISOString().split('T')[0]}
- "next week" → ${new Date(Date.now() + 7*86400000).toISOString().split('T')[0]}
- "March 15" → 2026-03-15

Keep responses brief and helpful. Don't repeat yourself.`

    try {
      const apiKey = import.meta.env.VITE_GEMINI_API_KEY
      
      if (!apiKey) {
        return "Please add your free Gemini API key to use the AI assistant. Get one at: https://aistudio.google.com/app/apikey"
      }
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [
            ...
            { role: 'model', parts: [{ text: 'Got it! I\'ll help you manage your assignments. What would you like to do?' }] },
            ...conversationHistory,
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (text && text.length > 0 && !text.includes('�')) {
          const response = await sendToAI(`I just uploaded a file called "${file.name}". Please extract ALL assignments, homework, projects, exams, quizzes, and anything with a due date from this content. Add each one to my calendar with the correct date.\n\nFile contents:\n${text.substring(0, 8000)}`)
          setMessages(prev => [...prev, { role: 'assistant', content: response }])
        } else {
          setMessages(prev => [...prev, { 
            role: 'assistant', 
            content: `I had trouble reading "${file.name}". The file might be corrupted or in an unsupported format. Try copying the text and pasting it directly into the chat.` 
          }])
        }
      } catch (error) {
        ...
        setMessages(prev => [...prev, { 
          role: 'assistant', 
          content: `Couldn't parse "${file.name}". Error: ${error.message}. Try pasting the content directly into the chat instead!` 
        }])
      }
