...
function CalendarApp({ user, onSignOut }) {
  ...
  const parseAssignmentsFromText = (text) => {
    const foundAssignments = []
    const classKeywords = {
      'math': ['math', 'algebra', 'calculus', 'geometry', 'statistics'],
      'science': ['science', 'biology', 'chemistry', 'physics', 'lab'],
      'english': ['english', 'essay', 'writing', 'literature', 'reading'],
      'history': ['history', 'social studies', 'government', 'civics'],
      'art': ['art', 'drawing', 'painting', 'design', 'creative']
    }

    const datePattern = /(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s*\d{4})?|\d{1,2}\/\d{1,2}(?:\/\d{2,4})?/gi
    
    const lines = text.split('\n')
    let currentClass = 'General'

    lines.forEach(line => {
      for (const [category, keywords] of Object.entries(classKeywords)) {
        if (keywords.some(k => line.toLowerCase().includes(k))) {
          currentClass = category.charAt(0).toUpperCase() + category.slice(1)
          break
        }
      }
      ...
      if (dateMatches) {
        dateMatches.forEach(dateStr => {
          ...
          if (parsedDate) {
            ...
            if (!title || title.length < 3) title = 'Assignment'
            
            let category = 'default'
            for (const [cat, keywords] of Object.entries(classKeywords)) {
              if (keywords.some(k => line.toLowerCase().includes(k) || title.toLowerCase().includes(k))) {
                category = cat
                break
              }
            }
  ...
  const handleFileUpload = async (files) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const parsed = parseAssignmentsFromText(text)
    ...
    if (totalAdded > 0) {
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `Found and added ${totalAdded} assignment${totalAdded > 1 ? 's' : ''} from your file!`
      }])
    } else {
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: "Couldn't find any dates in that file. Try a text file with clear due dates, or add assignments manually."
      }])
    }
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <header className="header">
          ...
          <div className="header-actions">
            <button className="icon-btn" onClick={() => setShowUploadModal(true)} title="Upload syllabus">
              <Icons.Upload />
            </button>
            <button className="icon-btn desktop-only" onClick={() => setShowSidebar(!showSidebar)} title="Toggle assistant">
              <Icons.PanelRight />
            </button>
            ...
            <div className="section-header">
              <div className="month-row">
                ...
                <div className="month-nav">
                  ...
                  <button className="nav-btn today-btn" onClick={goToToday} title="Go to today">
                    Today
                  </button>
                  <button className="nav-btn" onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1))}>
                    <Icons.ChevronRight />
      ...
      {showUploadModal && (
        <div className="modal-overlay" onClick={() => setShowUploadModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2 className="modal-title">Upload syllabus</h2>
            <p className="modal-subtitle">Upload a text file with your assignments and due dates</p>
            ...
            >
              ...
              <div className="drop-zone-text">Click to browse files</div>
              <div className="drop-zone-hint">TXT files work best</div>
            </div>
